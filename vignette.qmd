...
This analysis confirms our initial thought that `chlorides`, `fixed acidity`, `volatile acidity`, `residual sugar`, and `sulphates` are the most heavily skewed. However, most of the variables all have some moderate level of skew, with the exception of `citric acid`, `total sulfur dioxide`, `pH`, and `quality`. This means that all of these variables will have to be transformed in some way to make their distribution more normal. 
